...
import { ZoomIn, ZoomOut, Image, ImageDown, Trash2, Timer, Maximize } from "lucide-react";
...
export default function PreviewPane() {
  ...
  const [fullscreenImage, setFullscreenImage] = useState<{ str: string; format: string } | null>(
    null,
  );
    ...
    <>
      <div className="w-80 h-full bg-white border-l border-gray-200 flex flex-col flex-shrink-0">
        ...
        <div className="flex-1 flex flex-col min-h-0 border-b border-gray-200">
          ...
          <ZoomControls
            ...
            onFullscreen={() => {
              if (originalImage) {
                setFullscreenImage({ str: originalImage, format: imageFormat });
              }
            }}
          />
        </div>
        ...
        <div className="flex-1 flex flex-col min-h-0">
          ...
          <ZoomControls
            ...
            onFullscreen={() => {
              if (processedImage) {
                setFullscreenImage({ str: processedImage, format: imageFormat });
              }
            }}
          />
        </div>
      ...
      {fullscreenImage && (
        <ImageFullscreenPreview
          imageStr={fullscreenImage.str}
          format={fullscreenImage.format}
